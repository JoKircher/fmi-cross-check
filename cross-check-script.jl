...
# BouncingBalls_sf, TriggeredSubsystems_sf, TestModel1_sf
...
errorsDS = [[0.000005, 0.0005, 0.0005, 0.0005, 0.0005], [0.000005, 5e-6, 5e-6, 5e-6], [0.000005, 9, 3.1, 1, 0.00005, 0.00005, 0.11], [0.02, 0.5, 0.5]]
    ...
    for vendor in vendors
        ...
        for fmuname in FMUs
            ...
            if isfile("fmus/2.0/$type/$system/$vendor/$fmuname/$in.csv")               
                ...
                if type == "cs"
                    data = fmiSimulate(fmu,t_start, t_stop;recordValues=name[2:end], saveat=rowsres.time, setup=true)
                    df = DataFrame()
                    df.time = fmi2SimulationResultGetTime(data)
                    for i in 2:length(name)
                        insertcols!(df, i, Symbol(name[i]) => fmi2SimulationResultGetValues(data, name[i]))
                    end
                else
                    @warn "Noch nicht implementiert"
                end
